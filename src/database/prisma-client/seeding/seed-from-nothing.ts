...
export async function seedFromNothing(client: PlaygroundDbClient) {
    try {
    ...
    await createMany(client, 'user', randomUserData);
}

function generateRandomUserData(
    randomUserCount: number,
): ReadonlyArray<CreateModelEntryData<PlaygroundDbClient, 'user'>> {
    return Array(randomUserCount)
        .fill(0)
        .map((): CreateModelEntryData<PlaygroundDbClient, 'user'> => {
...
): ReadonlyArray<Omit<CreateModelEntryData<PlaygroundDbClient, 'post'>, 'author'>> {
    return Array(randomPostCount)
        .fill(0)
        .map((): Omit<CreateModelEntryData<PlaygroundDbClient, 'post'>, 'author'> => {
            return {
                title: uuid(),
            };
        });
